...
import { Constructor, Opt } from '@root/Utils/Typings'
...
export abstract class EthBaseAdapter extends BaseAdapter {
  protected etherClient: ethers.providers.BaseProvider = new ethers.providers.CloudflareProvider()
  ...
  constructor(nativeToken: Currency, chainId: EthChainIds, explorerUrl: string) {
    super(nativeToken, explorerUrl)
    this.chainId = chainId
  }

  setProvider(providerClass: Constructor<ethers.providers.BaseProvider>) {
    this.etherClient = new providerClass()
  }
  async initializeContract(contractAddress: Address, abi: ContractInterface) {
    if (this.contracts[contractAddress] || contractAddress === this.nativeToken.address) {
      return
    }
  ...
  async initializeToken(tokenAddress: Address, abi: ContractInterface = ERC20ABI) {
  ...
  async execute<T = any>(
    ...
    if (contractAddress === this.nativeToken.address && ['allowance', 'approve'].includes(method)) {
  ...
  async getBalance(targetAddress: Address = this.address): Promise<AdapterBalance> {
    if (!this.isConnected()) {
  ...
  async waitForTransaction(transactionHash: string): Promise<TransactionResult> {
  ...
  async initEventController() {
    this.etherClient.on('accountsChanged', ([address]: string[]) => {
      this.emitter.emit('AddressChanged', address)
    })

    this.etherClient.on('chainChanged', (chainId: string) => {
...
function computeInvocationParams(
  ...
  const reducedContractParameters = [...(args || []), { value: callValue ?? 0, ...gasOptions }]
