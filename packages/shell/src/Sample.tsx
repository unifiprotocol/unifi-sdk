...
export const SampleWithRoutes: ShellWrappedComp = (props) => {
  return (
    <Routes>
      <Route path="/sample-1/*" element={<Sample {...props} title="App 1" />} />
      <Route path="/sample-2/*" element={<Sample {...props} title="App 2" />} />
      <Route path="*" element={<Navigate to="/sample-1" />} />
    </Routes>
  );
};

export const Sample: ShellWrappedComp<{ title: string }> = ({
  title,
  connection,
  eventBus,
}) => {
  ...
  return (
    <div>
      <h1>{title}</h1>
      {address}
      <PrimaryButton onClick={() => navigate("liquidity")}>
        Go to Liquidity
      </PrimaryButton>
      ...
      <Routes>
        ...
        <Route index element={<Navigate to="exchange" />}></Route>
      </Routes>
    </div>
  );
};
const SampleSidebarbyApp: ShellWrappedComp<{ basePath: string }> = ({
  basePath,
}) => {
  const navigate = useNavigate();
  ...
  return (
    <>
      ...
      <SidebarItem
        ...
        icon={BiHome}
        onClick={() => navigate(`${basePath}/exchange`)}
      >
        Exchange
      </SidebarItem>
      <SidebarItem
        ...
        icon={BiHome}
        onClick={() => navigate(`${basePath}/liquidity`)}
      >
        Liquidity
      </SidebarItem>
      <SidebarItem
...
export const SampleSidebar: ShellWrappedComp = (props) => {
  return (
    <>
      <SampleSidebarbyApp {...props} basePath="/sample-1" />
      <SampleSidebarbyApp {...props} basePath="/sample-2" />
    </>
  );
};
